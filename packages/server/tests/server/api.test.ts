...
describe('GET /api/spaces/:id', () => {
  ...
  it('returns 404 when space not found', async () => {
    const res = await api.get('/api/spaces/nonexistent-id');
...
describe('PATCH /api/spaces/:id', () => {
  it('renames a space', async () => {
    await fixture.addSpace('Old Name').execute();
    const id = fixture.spaceIds[0];

    const res = await api.patch(`/api/spaces/${id}`, { name: 'New Name' });
    expect(res.status).toBe(200);
    const body = await api.json(res);
    expect(body.name).toBe('New Name');
    expect(body.subdomain).toBe('old-name');
  });

  it('returns 404 when space not found', async () => {
    const res = await api.patch('/api/spaces/nonexistent', {
      name: 'Anything',
    });
    expect(res.status).toBe(404);
  });
...
describe('DELETE /api/spaces/:id', () => {
  ...
  it('returns 404 when space not found', async () => {
    const res = await api.delete('/api/spaces/nonexistent-id');
    expect(res.status).toBe(404);
  });
});
...
describe('POST /api/backup', () => {
  it('creates a backup and returns filename', async () => {
    await fixture.addSpace('Test').execute();
  ...
  it('returns 400 when subdomain is missing', async () => {
    ...
    expect(res.status).toBe(400);
    expect(await api.json(res)).toEqual({
      error: 'Subdomain and elements required',
    });
  });
...
describe('GET /api/backups/:filename', () => {
  it('downloads backup as .excalidraw file', async () => {
    ...
    const filename = fixture.backups[0].filename;
    const res = await api.get(`/api/backups/${filename}`);
    expect(res.status).toBe(200);
  ...
  it('returns 404 when backup not found', async () => {
    const res = await api.get('/api/backups/nonexistent-file.excalidraw');
    expect(res.status).toBe(404);
  });
});
