...
- **Persistent deploy key** (replaced ephemeral-per-run): the `excalihub-e2e` deploy key on `jlai403/excalihub-ci` is registered ONCE and reused forever. `beforeAll` lists keys and reuses one whose fingerprint matches the app's current pubkey; it only registers (after sweeping stale `excalihub-e2e` keys) when no match exists — self-healing. `afterAll` never deletes the key. This kills the per-run key-churn emails (GitHub has NO setting to suppress deploy-key emails on personal repos — the toggle is org-owner-only). Key match compares `type + base64` tokens only (GitHub strips the trailing comment from `key`)
- **Stable SSH keypair via secret, NOT committed**: `globalSetup.ts` seeds `data-e2e/git-config/id_ed25519{,.pub}` from `E2E_SSH_PRIVATE_KEY` (writes key with trailing-newline normalization, chmod 600, derives pubkey via `ssh-keygen -y`); server's `GET /api/git/ssh-key` then returns the stable pubkey. Local: 1P field `e2e ssh key` on item `example-ssh-key` → `.env.schema` `exec('op read ...')`. CI: `secrets.E2E_SSH_PRIVATE_KEY` on `jlai403/excalihub`. Spec skips (not fails) when `E2E_GIT_TOKEN` OR `E2E_SSH_PRIVATE_KEY` absent
- **Token provisioning**: `E2E_GIT_TOKEN` (fine-grained PAT: `Administration: R/W` for deploy keys + `Contents: R/W`) stored as repo secret + 1Password item; resolved locally via `.env.schema` → `exec('op read "op://vault/item/field"')` (varlock plugin `op()` form does NOT resolve — use `exec()`)
...
- CI e2e job: `concurrency: ci-e2e` (no cancel-in-progress), `env: E2E_GIT_TOKEN` + `E2E_GIT_REPO=jlai403/excalihub-ci` + `E2E_SSH_PRIVATE_KEY`
...
- **New env**: `E2E_SSH_PRIVATE_KEY` — repo secret on `jlai403/excalihub` (CI), 1P field `e2e ssh key` on item `example-ssh-key` (local via varlock). Spec skips unless `E2E_GIT_TOKEN` AND `E2E_SSH_PRIVATE_KEY` are both set
