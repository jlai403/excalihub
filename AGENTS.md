...
- **New env**: `E2E_SSH_PRIVATE_KEY` — repo secret on `jlai403/excalihub` (CI), 1P field `e2e ssh key` on item `github PAT jlai403-excalihub-ci` (local via varlock). Spec skips unless `E2E_GIT_TOKEN` AND `E2E_SSH_PRIVATE_KEY` are both set
